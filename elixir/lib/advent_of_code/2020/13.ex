defmodule AdventOfCode.Y2020.D13 do
  @moduledoc """
  ...
  @real_input Input.raw("2020/13.txt")

  def solution1(input) do
    [timeslot, bus_ids] = String.split(input, "\n", trim: true)
    timeslot = String.to_integer(timeslot)
    bus_ids = String.split(bus_ids, ",") |> Enum.reject(& &1 == "x") |> Enum.map(&String.to_integer/1)

    {bus_line, wait} =
      bus_ids
      |> Enum.map(fn b -> {b, Float.ceil(timeslot / b) * b - timeslot} end)
  ...
  def solution2(input) do
    ...
    |> Enum.reduce(&find_overlap/2)
    |> elem(1)
  end

  def find_overlap({interval1, offset1}, {interval, _offset} = acc) do
    new_offset =
      0
      |> Stream.iterate(& &1 + 1)
      |> Stream.map(&arrival_timestamp(acc, &1))
      |> Enum.find(&has_arrival_at?(&1 + offset1, interval1))

    {interval1 * interval, new_offset}
  end

  def arrival_timestamp({interval, offset}, index) do
    interval * index + offset
  end

  def has_arrival_at?(timestamp, interval) do
    rem(timestamp, interval) == 0
  end
