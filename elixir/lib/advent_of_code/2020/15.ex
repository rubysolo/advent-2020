defmodule AdventOfCode.Y2020.D15 do
  @moduledoc """
  ...
  @real_input Input.raw("2020/15.txt")

  def solution1(input, iteration \\ 2020) do
    seed =
      input
    ...
    |> Map.merge(%{next_turn: Enum.count(seed) + 1, stream: Enum.reverse(seed)})
    |> iterate_to(iteration)
  end

  def iterate_to(%{next_turn: nt, stream: [last_spoken|_]}, iteration) when nt == iteration + 1, do: last_spoken
  def iterate_to(%{next_turn: nt, stream: [last_spoken|_] = stream} = state, iteration) do
    next_number =
      state
      |> Map.get(last_spoken)
      |> turn_diff()
    ...
    state
    |> Map.put(next_number, [nt | Map.get(state, next_number, [])])
    |> Map.merge(%{next_turn: nt + 1, stream: [next_number | stream]})
    |> iterate_to(iteration)
  end

  def turn_diff([h,p|_]), do: h - p
  def turn_diff(_), do: 0
