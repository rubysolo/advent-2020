defmodule AdventOfCode.Y2020.D4 do
  @moduledoc """
  ...
  @real_input Input.raw("2020/4.txt")
  ...
  def parse_passport(raw_passport) do
    ~r/\s+/
    |> Regex.split(raw_passport, trim: true)
    |> Enum.map(fn
      "" ->
        {}
      pair ->
  ...
  def has_fields?(%{} = passport) do
    Enum.all?(@fields -- [:cid], & Map.has_key?(passport, &1))
  end
  ...
  def analyze_field(passports, field) do
    all_values =
      passports
      |> Enum.map(& Map.get(&1, field))
      |> Enum.map(fn v -> {v, valid?(field, v)} end)

    {valid, invalid} = Enum.split_with(all_values, fn {_, valid} -> valid end)

    IO.puts("valid #{field} values: #{inspect unique_values(valid), limit: :infinity}")
    IO.puts("invalid #{field} values: #{inspect unique_values(invalid), limit: :infinity}")
  end

  def unique_values(tuples) do
    tuples
  ...
  def valid?(:byr, byr) do
    birth_year = String.to_integer(byr)
    Enum.member?(Range.new(1920, 2002), birth_year)
  end

  def valid?(:iyr, iyr) do
    issue_year = String.to_integer(iyr)
    Enum.member?(Range.new(2010, 2020), issue_year)
  end

  def valid?(:eyr, eyr) do
    exp_year = String.to_integer(eyr)
    Enum.member?(Range.new(2020, 2030), exp_year)
  end

  def valid?(:hgt, hgt) do
    digits = Regex.replace(~r/[^0-9]/, hgt, "")
    height = String.to_integer(digits)

    cond do
      String.ends_with?(hgt, "in") ->
        Enum.member?(Range.new(59, 76), height)
      String.ends_with?(hgt, "cm") ->
        Enum.member?(Range.new(150, 193), height)
      :otherwise ->
        false
    end
  end

  def valid?(:hcl, hcl) do
    Regex.match?(~r/\A#[0-9a-f]{6}\z/, hcl)
  end

  def valid?(:ecl, ecl) do
    Enum.member?(~w(amb blu brn gry grn hzl oth), ecl)
  end

  def valid?(:pid, pid) do
    Regex.match?(~r/\A[0-9]{9}\z/, pid)
  end
