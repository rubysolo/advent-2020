...
charToCell c =
  case c of
    ...
    'L' -> Empty
    '#' -> Occupied
    _ -> Debug.crash "this shouldn't happen"
...
increment { grid } =
  let
    updated =
      Array.indexedMap (incrementRow grid) grid
  in
  { model | grid = updated }

incrementRow : Grid -> Int -> Array Cell -> Array Cell
incrementRow grid rowIndex row =
  Array.indexedMap (incrementCell grid rowIndex) row

incrementCell : Grid -> Int -> Int -> Cell -> Cell
incrementCell grid rowIndex cellIndex cell =
  case cell of
    Floor -> Floor
    Empty ->
      if (occupiedNeighborCount grid (rowIndex, cellIndex)) == 0 then
        Occupied
      else
        Empty
    Occupied ->
      if (occupiedNeighborCount grid (rowIndex, cellIndex)) >= 4 then
        Empty
      else
        Occupied

occupiedNeighborCount : Grid -> (Int, Int) -> Int
occupiedNeighborCount grid (row, col) =
  let
    rows =
    ...
    coords =
      ...
      |> List.filter (\(r, c) -> r /= row || c /= col)

    cells =
      List.map (getCell grid) coords

    occupied =
      List.filter (\c -> c == Occupied) cells
  in
...
renderGrid { grid } =
  pre
    []
    (Array.map renderRow grid |> Array.toList)
