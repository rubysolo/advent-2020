...
import Day2 exposing (PasswordPolicy)
...
-- Passport is a validated passport record
...
passportChunks : String -> List String
passportChunks input =
    input
        |> String.split "\n\n"


parseFields : String -> List ( String, String )
parseFields input =
    input
        |> String.words
        |> List.sort
...
parsePassport : String -> Result (List Parser.DeadEnd) Passport
parsePassport input =
    input
        |> String.words
        |> List.sort
        |> String.join " "
        |> Parser.run passportParser
...
parseIntField : String -> Parser Int
parseIntField key =
...
parseStringField : String -> Parser String
parseStringField key =
    Parser.succeed identity
        |. Parser.keyword key
        |. Parser.symbol ":"
        |= (Parser.Advanced.chompWhile (not << isWhitespace) |> Parser.getChompedString)


parseMaybeStringField : String -> Parser (Maybe String)
parseMaybeStringField key =
    Parser.oneOf
        [ Parser.map Just <| parseStringField key
        , Parser.map (\_ -> Nothing) (Parser.succeed ())
        ]


parseEyeColorField : String -> Parser EyeColor
parseEyeColorField key =
    parseStringField key
        |> Parser.andThen parseEyeColor
...
parseEyeColor raw =
    case raw of
        "amb" ->
            Parser.succeed Amber

        "blu" ->
            Parser.succeed Blue

        "brn" ->
            Parser.succeed Brown
        ...
        "hzl" ->
            Parser.succeed Hazel

        "oth" ->
            Parser.succeed Other

        _ ->
            Parser.problem <| "invalid eye color: " ++ raw


parseHairColorField : String -> Parser String
parseHairColorField key =
    Parser.succeed identity
        |. Parser.keyword key
        |. Parser.symbol ":"
        |. Parser.symbol "#"
        |= (Parser.Advanced.chompWhile Char.isHexDigit |> Parser.getChompedString)


parseHeightField : String -> Parser Height
parseHeightField key =
    parseStringField key
        |> Parser.andThen parseHeight
...
parseHeight raw =
    let
        ...
        digits =
            String.dropRight 2 raw

        value =
            String.toInt digits |> Maybe.withDefault 0
    in
    case suffix of
        "cm" ->
            Parser.succeed <| Centimeter value

        "in" ->
            Parser.succeed <| Inch value

        _ ->
            Parser.problem <| "invalid height value: " ++ raw


passportParser : Parser Passport
passportParser =
    Parser.succeed Passport
        |= parseIntField "byr"
        |. Parser.spaces
        |= parseMaybeStringField "cid"
        |. Parser.spaces
        |= parseEyeColorField "ecl"
        |. Parser.spaces
        |= parseIntField "eyr"
        |. Parser.spaces
        |= parseHairColorField "hcl"
        |. Parser.spaces
        |= parseHeightField "hgt"
        |. Parser.spaces
        |= parseIntField "iyr"
        |. Parser.spaces
        |= parseStringField "pid"
        |. Parser.spaces


validatePassport : Passport -> Bool
validatePassport passport =
    List.all (\validator -> validator passport)
        [ birthYearValid
        , issueYearValid
        , expirationYearValid
        , heightValid
...
birthYearValid { birthYear } =
    List.member birthYear <| List.range 1920 2002


issueYearValid : Passport -> Bool
issueYearValid { issueYear } =
    List.member issueYear <| List.range 2010 2020


expirationYearValid : Passport -> Bool
expirationYearValid { expirationYear } =
    List.member expirationYear <| List.range 2020 2030
...
heightValid { height } =
    case height of
        Centimeter cm ->
            List.member cm <| List.range 150 193

        Inch inch ->
            List.member inch <| List.range 59 76


hairColorValid : Passport -> Bool
hairColorValid { hairColor } =
    String.length hairColor == 6


passportIDValid : Passport -> Bool
passportIDValid { passportId } =
    String.length passportId == 9


requiredPassportFields : Set String
                ...
                let
                    presentFields =
                        kvPairs
                            ...
                            |> Set.fromList
                in
                Set.intersect presentFields requiredPassportFields == requiredPassportFields
            )
        |> List.length
